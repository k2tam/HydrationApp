...
class HydrationModel: ObservableObject{
    ...
    //Health Model (HealthModel.swift)
    @Published var health:HealthModel
    
    
    //Intake entries (WaterData.swift)
    @Published private(set) var intake:[WaterData]{
        //When modified, call the save function
        didSet{
            
            let currentDay = DateHelper.getCurrentDay(day: Date())
            self.intake.filter(){DateHelper.getCurrentDay(day: $0.date) == DateHelper.getCurrentDay(day: Date())}
    ...
    //Recalcualtes to progress double
